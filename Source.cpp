...
void szukaj_link_praca(GumboNode* node,int& licznik) { //Funkcja do wypisania linkow do pliky tekstowego
	...
	for (unsigned int i = 0; i < children->length; i++) {
		szukaj_link_praca(static_cast<GumboNode*>(children->data[i]),licznik);
	}
}
...
int main() {
	...
	szukaj_link_praca(zawartosc_gumbo->root,licznik);
